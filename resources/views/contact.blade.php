<!-- resources/views/votings/contact.blade.php -->
<x-guest-layout>
    <x-slot name="title">Contact Us</x-slot>

    @vite(['resources/css/contact.css', 'resources/js/app.js'])

    <div class="page-container">
        <!-- Background Image -->
        <img src="https://cdn.builder.io/api/v1/image/assets/TEMP/fe992e701c7edc01644f69af503f11ed319f8132"
            alt="CSG Logo" class="background-logo" />

        <!-- Contact Information -->
        <section class="contact-container">
            <h1 class="contact-heading">Contact Us</h1>
            <p class="contact-description">
                Feel free to use the form or drop us an email. Old-fashioned phone calls
                work too.
            </p>
            <div class="contact-info-container">

                <!-- Phone Contact -->
                <div class="contact-info-item">
                    <div class="icon-wrapper">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                            class="icon">
                            <path
                                d="M15.6 14.522C13.205 17.042 7.096 10.988 9.5 8.45799C10.968 6.91299 9.31 5.14799 8.392 3.84899C6.669 1.41399 2.888 4.77599 3.002 6.91499C3.365 13.661 10.662 21.655 17.728 20.957C19.938 20.739 22.478 16.747 19.943 15.288C18.675 14.558 16.934 13.118 15.6 14.521"
                                stroke="black" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                    </div>
                    <span class="contact-text">385-729-416</span>
                </div>

                <!-- Email -->
                <div class="contact-info-item">
                    <div class="icon-wrapper">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                            class="icon">
                            <path
                                d="M20 4H4C2.897 4 2 4.897 2 6V18C2 19.103 2.897 20 4 20H20C21.103 20 22 19.103 22 18V6C22 4.897 21.103 4 20 4ZM20 6V6.511L12 12.734L4 6.512V6H20ZM4 18V9.044L11.386 14.789C11.5611 14.9265 11.7773 15.0013 12 15.0013C12.2227 15.0013 12.4389 14.9265 12.614 14.789L20 9.044L20.002 18H4Z"
                                fill="black"></path>
                        </svg>
                    </div>
                    <span class="contact-text">csghelp@umindanao.edu.ph</span>
                </div>

                <!-- Address -->
                <div class="contact-info-item">
                    <div class="icon-wrapper">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                            class="icon">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M12 2C14.3869 2 16.6761 2.94821 18.364 4.63604C20.0518 6.32387 21 8.61305 21 11C21 14.074 19.324 16.59 17.558 18.395C16.6757 19.2871 15.7129 20.0958 14.682 20.811L14.256 21.101L14.056 21.234L13.679 21.474L13.343 21.679L12.927 21.921C12.6446 22.0822 12.3251 22.1669 12 22.1669C11.6749 22.1669 11.3554 22.0822 11.073 21.921L10.657 21.679L10.137 21.359L9.945 21.234L9.535 20.961C8.42283 20.2085 7.3869 19.3491 6.442 18.395C4.676 16.589 3 14.074 3 11C3 8.61305 3.94821 6.32387 5.63604 4.63604C7.32387 2.94821 9.61305 2 12 2ZM12 4C10.1435 4 8.36301 4.7375 7.05025 6.05025C5.7375 7.36301 5 9.14348 5 11C5 13.322 6.272 15.36 7.871 16.996C8.55853 17.692 9.30166 18.3308 10.093 18.906L10.551 19.232C10.699 19.3353 10.8413 19.4313 10.978 19.52L11.368 19.77L11.711 19.979L12 20.148L12.455 19.879L12.822 19.649C13.0173 19.525 13.2263 19.386 13.449 19.232L13.907 18.906C14.6983 18.3308 15.4415 17.692 16.129 16.996C17.728 15.361 19 13.322 19 11C19 9.14348 18.2625 7.36301 16.9497 6.05025C15.637 4.7375 13.8565 4 12 4ZM12 7C13.0609 7 14.0783 7.42143 14.8284 8.17157C15.5786 8.92172 16 9.93913 16 11C16 12.0609 15.5786 13.0783 14.8284 13.8284C14.0783 14.5786 13.0609 15 12 15C10.9391 15 9.92172 14.5786 9.17157 13.8284C8.42143 13.0783 8 12.0609 8 11C8 9.93913 8.42143 8.92172 9.17157 8.17157C9.92172 7.42143 10.9391 7 12 7ZM12 9C11.4696 9 10.9609 9.21071 10.5858 9.58579C10.2107 9.96086 10 10.4696 10 11C10 11.5304 10.2107 12.0391 10.5858 12.4142C10.9609 12.7893 11.4696 13 12 13C12.5304 13 13.0391 12.7893 13.4142 12.4142C13.7893 12.0391 14 11.5304 14 11C14 10.4696 13.7893 9.96086 13.4142 9.58579C13.0391 9.21071 12.5304 9 12 9Z"
                                fill="black"></path>
                        </svg>
                    </div>
                    <span class="contact-text">MacArthur Highway, Matina, Davao City, Philippines</span>
                </div>
            </div>
        </section>

        <!-- Contact Fields -->
        <main class="contactField-container">
            <form class="contact-form">
                <div class="form-group">
                    <label for="name" class="form-label">Name</label>
                    <div class="name-fields">
                        <div class="field-wrapper">
                            <div class="input-container">
                                <input type="text" id="first-name" placeholder="First" class="form-input" />
                            </div>
                        </div>
                        <div class="field-wrapper">
                            <div class="input-container">
                                <input type="text" id="last-name" placeholder="Last" class="form-input" />
                            </div>
                        </div>
                    </div>
                </div>

                <div class="form-section">
                    <label for="email" class="form-label">Email</label>
                    <div class="input-container">
                        <input type="email" id="email" placeholder="example@umindanao.edu.ph" class="form-input" />
                    </div>
                </div>

                <div class="form-section">
                    <label for="phone" class="form-label">Phone (optional)</label>
                    <div class="input-container">
                        <input type="tel" id="phone" placeholder="xxx-xxx-xxxx" class="form-input" inputmode="numeric"
                            pattern="[0-9]*" oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
                    </div>
                </div>

                <div class="form-section">
                    <label for="message" class="form-label">Message</label>
                    <div class="message-container">
                        <span class="message-placeholder">Type a message ...</span>
                        <textarea id="message" class="form-textarea"></textarea>
                    </div>
                </div>

                <button type="submit" class="submit-button">Submit</button>
            </form>
        </main>

    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
        const textarea = document.getElementById("message");
        const placeholder = document.querySelector(".message-placeholder");

        textarea.addEventListener("input", () => {
            if (textarea.value.trim() !== "") {
                placeholder.style.opacity = "0";
            } else {
                placeholder.style.opacity = "1";
            }
        });
    </script>
</x-guest-layout>